...
export default function NBAPage() {
  ...
  const refreshPage = async () => {
    ...
    trackRefreshOutcome("/nba", refreshStartedAt, {
  ...
  useEffect(() => {
    trackStaleCache("/nba", lastUpdated);
  }, [lastUpdated]);
  ...
  return (
    <div>
      ...
      {!hasNbaData && refreshFailed ? (
        <ErrorState
          title="NBA board unavailable"
          message="BetMate could not load a usable NBA snapshot yet. Try again shortly while the latest cache is retried."
          tone="danger"
      ...
      <ErrorBoundary sectionName="NBA opportunities">
        <BestNbaOpportunities opportunities={nbaOpportunities} />
      </ErrorBoundary>

      <ErrorBoundary sectionName="NBA predictions">
        <div className="game-cards-list">
          {games.map((game) => {
            ...
            const matchupData: MatchupDrawerData = {
              ...
              subTitle: formatGameStartLabel(game.date),
              date: game.date,
              outcomes: [
                {
                  name: game.home_team,
              ...
              metadata: {
                restDays: game.features?.home_b2b || game.features?.away_b2b
                  ? `Home: ${game.features?.home_b2b ? "B2B" : "Rested"} / Away: ${game.features?.away_b2b ? "B2B" : "Rested"}`
                  : undefined,
                form: game.features?.home_ortg && game.features?.away_ortg
                  ? `ORTG - Home: ${game.features.home_ortg} / Away: ${game.features.away_ortg}`
                  : undefined,
                confidenceSignal,
                urgencySignal,
              },
