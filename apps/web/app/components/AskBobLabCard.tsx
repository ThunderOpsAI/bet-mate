...
export default function AskBobLabCard({
  ...
  return (
    <div className="bg-slate-950/90 border border-slate-800 rounded-xl p-4 shadow-xl space-y-3 relative overflow-hidden">
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        {SUGGESTED_PROMPTS.map((prompt, idx) => (
          <button
            ...
            className="text-[11px] px-2 py-0.5 rounded-full bg-slate-900/80 border border-slate-800 text-slate-300 hover:text-emerald-400 hover:border-emerald-500/40 transition-colors"
          >
            {prompt}
          </button>
        ))}
      ...
      <form onSubmit={onSubmit} className="flex gap-2">
        <div className="relative flex-1">
          <input
            ...
            type="text"
            className="w-full bg-slate-900/80 border border-slate-800 rounded-lg pl-3 pr-9 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500/50 transition-colors"
            value={chatInput}
        ...
        <button
          ...
          className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 disabled:text-slate-500 text-slate-900 font-bold text-xs rounded-lg flex items-center gap-1.5 transition-colors shrink-0 cursor-pointer disabled:cursor-not-allowed"
        >
          {chatLoading ? (
            <>
      ...
      {/* AI Reply Display */}
      {chatReply && (
        <div className="bg-slate-900/90 border border-emerald-500/20 rounded-lg p-3 text-xs text-slate-200 space-y-1.5 animate-fadeIn">
